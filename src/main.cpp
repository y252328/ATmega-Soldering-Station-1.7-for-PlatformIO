...
// 此v1.7+版本功能简介
// This 1.7+ version of the code implements:
// - 烙铁头温度实时监测
...
uint8_t MenuScreen(uint8_t selected) {
  ...
  do {
    ...
    //绘制图标 如果有指定的话
    for (byte i = 0; i < 5; i++) {
      if (selected - 2 + i >= 0 && selected - 2 + i < Menu_table[MenuLevel]) {
        DrawApp(-72 + i * 56 + SlidingAnimationX, selected - 2 + i + QueryMenuObject());
      }
    }
...
uint16_t getVIN() {
  ...
  return (result * Vcc / 185.5);      // 179.474 = 1023 * R13 / (R12 + R13)
}
